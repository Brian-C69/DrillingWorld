...
if (isset($_GET['cid']) && is_numeric($_GET['cid'])) {
    ...
    $delete_sql = "DELETE FROM calculators WHERE cid = ?";
    if ($delete_stmt = mysqli_prepare($link, $delete_sql)) {
        mysqli_stmt_bind_param($delete_stmt, "i", $user_id);
        if (mysqli_stmt_execute($delete_stmt)) {
            $_SESSION['success_message'] = "Calculator deleted successfully.";
        } else {
            $_SESSION['error_message'] = "Error deleting calculator.";
        }
    ...
    // Redirect back to AdminCalculatorPanel.php after deletion
